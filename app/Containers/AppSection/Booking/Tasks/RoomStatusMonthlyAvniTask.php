...
class RoomStatusMonthlyAvniTask extends ParentTask
{
    use HashIdTrait;
            ...
            foreach ($rooms as $room) {
                ...
                for ($day = 1; $day <= $daysInMonth; $day++) {
                    ...
                    if ($statusRecord) {
                        ...
                        //                 'status' => "booking"
                        //             ];
                        //     // $statusData = ['current_date' => $curdate, 'status' => "booking" ,'current_date_sh' => sprintf("%02d", $day) , 'current_day' => $dayOfWeek,];
                        // } elseif ($statusRecord->status === "checkin") {
                        //     $statusData = ['current_date' => $curdate, 'status' => "Check In" ,'current_date_sh' => sprintf("%02d", $day) , 'current_day' => $dayOfWeek,];
                        // }
                         elseif ($statusRecord->status === "checkout") {
                            $statusData = ['current_date' => $curdate, 'status' => "Check Out" ,'current_date_sh' => sprintf("%02d", $day) , 'current_day' => $dayOfWeek,];
                        } elseif ($statusRecord->status === "maintenance") {
                            $statusData = ['current_date' => $curdate, 'status' => "Maintenance" ,'current_date_sh' => sprintf("%02d", $day) , 'current_day' => $dayOfWeek,];
                        } elseif ($statusRecord->status === "house keeping") {
                            $statusData = ['current_date' => $curdate, 'status' => "House Keeping" ,'current_date_sh' => sprintf("%02d", $day) , 'current_day' => $dayOfWeek,];
                        } elseif ($statusRecord->status === "available") {
                            $statusData = ['current_date' => $curdate, 'status' => "Available" ,'current_date_sh' => sprintf("%02d", $day) , 'current_day' => $dayOfWeek,];
                        }
                    } else {
                        $format_cur_date = sprintf("%04d-%02d-%02d", $year, $month, $day);
                        $bookingRoom = Bookingroom::where('check_in', '<=', $format_cur_date)
                            ->where('check_out', '>=', $format_cur_date)
                            ->first();
                        
                        $bookingMaster = Booking::where('id', $booking_id)->first();
                        $booking = Bookingroom::where('booking_master_id', $bookingMaster->id)
                        ->first();
                        if ($bookingRoom) {
                            $statusData = ['current_date' => $curdate ,
                                        "booking_room_id" => $this->encode($bookingMaster->id),
                                        "booking_no" => $bookingMaster->booking_no,
                                        'current_date_sh' => sprintf("%02d", $day),
                                        'current_day' => $dayOfWeek,
