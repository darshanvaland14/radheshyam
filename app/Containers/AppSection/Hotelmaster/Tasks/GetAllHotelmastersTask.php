...
class GetAllHotelmastersTask extends ParentTask
{
    ...
    public function run($request)
    {
        try { 
            ...
            if ($request->query->has('isRoom')) {
                $isRoom = $request->query->get('isRoom');
            } else {
                $isRoom = null;
            }
            ...
            if ($isRoom !== null) {
                $getData = DB::table('hs_hotel_master')
                    ->join('hs_hotel_room', function ($join) {
                        $join->on('hs_hotel_master.id', '=', 'hs_hotel_room.hotel_master_id')
                            ->whereNull('hs_hotel_room.deleted_at');
                           
                    })
                    ->join('hs_hotel_pricing', function ($join) {
                        $join->on('hs_hotel_room.room_type_id', '=', 'hs_hotel_pricing.room_type_id')
                            ->on('hs_hotel_master.id', '=', 'hs_hotel_pricing.hotel_master_id'); // Ensures pricing matches hotel
                    })
                    ->select('hs_hotel_master.*')
                    ->distinct() // Ensures unique hotels (avoids duplicates if multiple rooms exist)
                    ->get();
                    // return "darshan";   
            } else {
                if($role_id == 1){
                    $getData = Hotelmaster::orderBy('name', 'asc')->get();
                }else{
                    $getData = Hotelmaster::orderBy('name', 'asc')->where('assign_to', $assign_id)->get();
                }
            }
            $returnData = array();
