    ...
    public function run($InputData)
    {
        ...
        try {
            ...
            if (($field_db == "") || ($field_db == NULL)) {
                if (!empty($per_page)) {
                    $getData = Tenantuser::orderBy('created_at', 'DESC')->paginate($per_page);
                } else {
                    $getData = Tenantuser::orderBy('created_at', 'DESC')->get();
                }
            } else {
                if ($field_db == "role_id") {
                ...
                if (!empty($per_page)) {
                    $getData = Tenantuser::where($field_db, 'like', '%' . $search_val . '%')->orderBy('created_at', 'DESC')->paginate($per_page);
                } else {
                    $getData = Tenantuser::where($field_db, 'like', '%' . $search_val . '%')->orderBy('created_at', 'DESC')->get();
                }
            }
            $returnData = array();
            if (!empty($getData) && count($getData) >= 1) {
                ...
                for ($i = 0; $i < count($getData); $i++) {
                    ...
                    $roleName = Roles::select('name')->where('id', $getData[$i]['role_id'])->first();
                    $returnData['data'][$i]['role_id'] = $this->encode($getData[$i]['role_id']);
                    $returnData['data'][$i]['role_name'] = $roleName->name;
                    ...
                    if (!empty($Tenantuserdetails)) {
                        $returnData['data'][$i]['permanent_address'] = $Tenantuserdetails['permanent_address'];
                        $returnData['data'][$i]['local_city'] = $Tenantuserdetails['local_city'];
                        $returnData['data'][$i]['local_state'] = $Tenantuserdetails['local_state'];
                        $returnData['data'][$i]['local_zipcode'] = $Tenantuserdetails['local_zipcode'];
                        $returnData['data'][$i]['local_country'] = $Tenantuserdetails['local_country'];
                        ...
                        // $returnData['data'][$i]['userdetails'][0]['permanent_address'] = $Tenantuserdetails['permanent_address'];
                        $returnData['data'][$i]['permanent_city'] = $Tenantuserdetails['permanent_city'];
                        $returnData['data'][$i]['permanent_state'] = $Tenantuserdetails['permanent_state'];
                        $returnData['data'][$i]['permanent_zipcode'] = $Tenantuserdetails['permanent_zipcode'];
                        $returnData['data'][$i]['permanent_country'] = $Tenantuserdetails['permanent_country'];
                        ...
                        $returnData['data'][$i]['permanent_address'] = NULL;
                        ...
                        $returnData['data'][$i]['local_address'] = NULL;
                        $returnData['data'][$i]['local_city'] = NULL;
                        $returnData['data'][$i]['local_state'] = NULL;
                        $returnData['data'][$i]['local_zipcode'] = NULL;
                        $returnData['data'][$i]['local_country'] = NULL;
