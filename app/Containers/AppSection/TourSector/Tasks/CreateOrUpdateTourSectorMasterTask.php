...
class CreateOrUpdateTourSectorMasterTask extends ParentTask
{
    use HashIdTrait;
    ...
    {
        try {   
            $tour_sector_id = $this->decode($request->tour_sector_id);
            if($tour_sector_id){  
                $createData = TourSector::find($tour_sector_id);
            }else{
                $existing = TourSector::where('name', $request->name)->first();
                if ($existing) {
                    return [
                        'result' => false,
                        'message' => 'Tour Sector already exists',
                        'object' => 'TourSector',
                        'data' => [],
                    ];
                }
            ...
            $returnData = [
                'result' => true,
                'message' => $tour_sector_id ? 'Tour Sector Updated Successfully' : 'Tour Sector Created Successfully',
                'object' => 'TourSector',
                //'data' => $createData,
            ];
