...
class GetAllGalleryItemsTask extends ParentTask
{
    use HashIdTrait;
    ...
    public function run()
    {
        try{
            ...
            $getData = TourWebGalleryItem::orderBy('id', 'desc')->get();
            ...
            if (!empty($getData) && count($getData) >= 1) {
                for ($i = 0; $i < count($getData); $i++) {
                    ...
                    $returnData['data'][$i]['gallery_item_id'] = $this->encode($getData[$i]->id);
                    $returnData['data'][$i]['title'] =  $getData[$i]->title;
                    $returnData['data'][$i]['category'] =  $getData[$i]->category;
                    $returnData['data'][$i]['location'] =  $getData[$i]->location;
                    $returnData['data'][$i]['src'] = $theme_setting->api_url . $getData[$i]->image;
                }
            } else {
